...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
	if(isset($_FILES["file"]["type"]) && isset($_POST['itemID']) && isset($_POST['sectionName']) &&
		...
		if (( ($_FILES["file"]["type"] == "image/png") || ($_FILES["file"]["type"] == "image/jpg") || ($_FILES["file"]["type"] == "image/jpeg")
			...
			echo "Return Code: " . $_FILES["file"]["error"] . "<br/><br/>";
				...
				if (file_exists("../upload/" . $_FILES["file"]["name"])) {
					...
					$n_id = trim($_POST['itemID']);
                    
                    $result = product_update($n_id,$n_name,$n_sec,$n_desc,$n_price,$n_img);
                    ...
                    if($result){
                        ...
                        $response['error'] = true;
                        $response['msg'] = $_POST['itemName'] . " <span id='invalid'><b>can't updateing item, please try again.</b></span> ";
                        die(json_encode($response));
                    }
				}else{
					...
					*/
					...
					unlink("../upload/".$_POST['itemIMG']); // delete the prev img from server
                    ...
                    if($result){
                        ...
                        $response['error'] = true;
                        $response['msg'] = $_POST['itemName'] . " <span id='invalid'><b>can't Update item, please try again.</b></span> ";
                        die(json_encode($response));
                    }
                    // end insert to db
