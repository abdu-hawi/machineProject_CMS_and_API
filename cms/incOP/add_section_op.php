		...
		if (( ($_FILES["file"]["type"] == "image/png") || ($_FILES["file"]["type"] == "image/jpg") || ($_FILES["file"]["type"] == "image/jpeg")
			...
			echo "Return Code: " . $_FILES["file"]["error"] . "<br/><br/>";
				...
				if (file_exists("../upload/" . $_FILES["file"]["name"])) {
                    ...
                    $item = section_get_by_name($_POST['sectionName']);
                    if($item != NULL)
                    {
                        machine_db_close();
                        $response['error'] = true;
                        $response['msg'] = $_POST['sectionName'] . " <span id='invalid'><b>already exists.</b></span> ";
                        die(json_encode($response));
                    }
                    ...
                    // insert to db
                    $n_name = trim($_POST['sectionName']);
                    $n_img = trim($i_img_name);
                    
                    $result = section_add($n_name,$n_img);
                    ...
                    if($result){
                        ...
                        $response['error'] = true;
                        $response['msg'] = $_POST['sectionName'] . " <span id='invalid'><b>can't insert item, please try again.</b></span> ";
                        die(json_encode($response));
                    }
                    // end insert to db
				}
