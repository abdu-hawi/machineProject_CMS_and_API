...
if(count($sJson) === 0){
	$response['error'] = true;
	$response['msg'] ='<div class="alert alert-danger col-lg-10"><b>Please add section item before add any items</b></div>';
	die( json_encode($response) );
}
...
$prodGet = product_get_by_id($_POST['id']);
$pJson = json_decode($prodGet, true);
foreach($pJson as $pi){
?>

<h2>Edit: <b><?php echo $pi['Name']; ?> </b></h2>
<hr><br/>
<form id="uploadimage" class="form-horizontal col-lg-10  col-md-12 col-sm-12 col-xs-12" action="" method="post" enctype="multipart/form-data">
    ...
    <div id="selectImage" class="form-group">
        <div class="panel-body">
			<select class="form-control" id="sectionName" name="sectionName">
			  ...
			  <?php foreach($sJson as $po){ 
						if(strcmp($po['ps_name'],$pi['Section']) == 0){
							echo '<option value="'.$po['ps_id'].'" selected>'.$po['ps_name'].'</option>';
						}else{
							echo '<option value="'.$po['ps_id'].'">'.$po['ps_name'].'</option>';
						}
					} 
				?>
			</select>
			<input type="hidden" value="<?php echo $pi['ID'] ?>" name="itemID"  required />
			<input type="hidden" value="<?php echo $pi['IMG'] ?>" name="itemIMG"  required />
			<input class="form-control" type="text" placeholder="Item name" value="<?php echo $pi['Name'] ?>" id="itemName" name="itemName"  required />
			<input class="form-control" type="text" placeholder="Discription" value="<?php echo $pi['Desc'] ?>" id="itemDesc" name="itemDesc"  required />
			<input class="form-control" type="number" step="0.01" min="0" placeholder="Price" value="<?php echo $pi['price'] ?>" id="itemPrice" name="itemPrice"  required />
    ...
    <div class="form-group">        
      <div class="col-lg-12">
      	<input type="submit" value="Updating Item" class="submit btn btn-info " />
      </div>
    </div>
...
$(document).ready(function (e) {
	...
	$("#uploadimage").on('submit',(function(e) {
		...
		$('#image_preview').show();
		$.ajax({
			url: "includeOP/edit_item_op.php", // Url to which the request is send
