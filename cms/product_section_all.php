...
$statment_product_section = mysqli_query($mp_handle, "SELECT * FROM `product_section`");
		...
		echo'
			...
			if( $row["isAvilable"] == 0){
				...
				<a href="#" id="'.$row["ps_name"].'" class="sec_parent">
				  ...
				  <div class="panel-footer">
					  <a href="#" id="'.$sec_id.'" class="activeSec pull-right">
						<span style="color:brown; font-size:16px;">Make it active</span>
					  </a>
					  <a href="#" id="'.$sec_id.'" class="editSec pull-left">
						<span style="color:blue; font-size:16px;">Edit</span>
					  </a>
...
// activeSec
$(document).on('click','.activeSec',function(){
	var id = $(this).attr("id");
	if(confirm("Are you sure you want to active?")){
		$.ajax({
			url:'active_product_section.php',
			type:'POST',
			data:'id='+id,
			success:function(data){
				if(data.error){
